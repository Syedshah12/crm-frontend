...
import {
  ...
  punchIn,
  punchOut,
} from "../../features/punchings/punchingSlice";
...
import PunchInModal from "../../components/shopAdmin/PunchInModal.jsx";

export default function PunchingPage() {
  const dispatch = useDispatch();
  const { darkMode } = useDarkMode();

  const { list: punchings, loading } = useSelector((state) => state.punchings);
  ...
  // Fetch punchings whenever filters change
  useEffect(() => {
    dispatch(
      fetchPunchings({
        shopId: filterShop,
  ...
  }, [filterShop, filterEmployee, dispatch]);

  const handlePunchOut = (punchingId) => {
    dispatch(
      punchOut({
        punchingId,
        punchOutDatetime: new Date().toISOString(),
      })
    ).then((res) => {
    ...
    <table className="w-full table-fixed rounded-lg">
      ...
      >
        <tr>
          ...
          <th className="p-4 text-[9px] md:text-sm text-left">Punch In</th>
          <th className="p-4 text-[9px] md:text-sm text-left">Punch Out</th>
      ...
      <tbody>
        {punchings.map((punch, index) => (
          ...
          >
            ...
            <td className="p-4 whitespace-normal text-[9px] md:text-sm">
              {new Date(punch.punchInDatetime).toLocaleString()}
            </td>
            <td className="p-4 whitespace-normal text-[9px] md:text-sm">
              {punch.punchOutDatetime
                ? new Date(punch.punchOutDatetime).toLocaleString()
                : "--"}
            </td>
            <td className="p-4 whitespace-normal text-sm flex items-center gap-3">
              {!punch.punchOutDatetime && (
                <button
                  onClick={() => handlePunchOut(punch._id)}
                  title="Punch Out"
                  className={`
                    group relative inline-flex items-center justify-center
                    md:w-10 w-8 md:h-10 h-8 rounded-xl
        ...
        {/* PUNCH IN MODAL */}
        <PunchInModal isOpen={punchModalOpen} onClose={() => setPunchModalOpen(false)} />
      </div>
    </ShopLayout>
  );
}
