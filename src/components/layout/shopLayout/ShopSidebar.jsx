...
import {  FiCalendar, FiDollarSign, FiClipboard, FiFileText } from "react-icons/fi";
...
const menuItems = [
  ...
  { name: "Employees", icon: <FiUsers />, path: "/shop/employees" },       // 👥 represents employees
  { name: "Rota", icon: <FiCalendar />, path: "/shop/create/rota" },       // 📅 calendar for shifts
  { name: "Punchings", icon: <FiClipboard />, path: "/shop/punchings" },   // 📝 clipboard for attendance
  { name: "Salary", icon: <FiDollarSign />, path: "/shop/salary" },        // 💵 salary/pay
  { name: "PayOut", icon: <FiFileText />, path: "/shop/payout" },          // 📄 payout record
];

export default function ShopSidebar({ isOpen, onClose, user }) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-transparent backdrop-blur-md  z-30 md:hidden"
          onClick={onClose}
        />
      )}
...
>
  ...
  {user && (
    <div className="px-6 py-4 border-t flex flex-col gap-3 transition-colors duration-500"
      ...
      {/* Dark Mode Toggle */}
      <motion.div
        className={`relative w-16 mb-2 h-8 flex items-center rounded-full cursor-pointer transition-colors duration-500
